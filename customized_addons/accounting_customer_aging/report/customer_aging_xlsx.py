...
class CustomerAgingXlsx(models.AbstractModel):
    _name = 'report.accounting_customer_aging.report_customer_aging_xls'
    ...
    def generate_xlsx_report(self, workbook, data, customers):
        ...
        worksheet.merge_range('C6:H6', "All Posted Entries", normal_normal_center)
        ...
        worksheet.merge_range('A6:B6', "Receivable Accounts", normal_normal_center)
        ...
        for customer in data:
            if customer in ["data", "context", "token", "Company Name", "Total Amount by range", "Aging Date"]:
                pass
            else:
                row += 1
                customer_list = [customer] + list(data[customer].values())
                worksheet.write(f'A{row}', num, table_header_center)
                worksheet.write(f"B{row}", customer_list[0], table_data_left)    
                worksheet.write_row(f"C{row}", customer_list[1:], table_data_center)
                num += 1
