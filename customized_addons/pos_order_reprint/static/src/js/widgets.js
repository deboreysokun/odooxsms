odoo.define("pos_order_reprint.widgets", function(require) {
    "use strict";
    ...
    var OrderListScreenWidget = ScreenWidget.extend({
        ...
        action_print: function(order_data, order) {
            ...
            // call push_order function to add Order Ref to receipt
            this.pos.push_order(order, {"reprint": true}).then(function() {
                self.gui.show_screen("receipt");
                self.pos.reloaded_order = false;
                // Set skip screen to whatever previous state
                self.pos.config.iface_print_skip_screen = skip_screen_state;

                // If it's invoiced, we also print the invoice
                if (order_data.to_invoice) {
                    self.pos.chrome.do_action("point_of_sale.pos_invoice_report", {
